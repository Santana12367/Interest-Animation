...
[![Thumbnail for my projet](rams.png)]([youtube-URL-here](https://www.youtube.com/watch?v=SrZMtO4jI0M))
https://www.youtube.com/watch?v=SrZMtO4jI0M


## Story Description

This animation captures the 2026 Rams season by transitioning from the team's iconic brand to a detailed look at the roster and its standout superstars. The project uses a 2D String array to organize the depth chart, linking specific positions to the starters and backups who defined the team's structure. Finally, a 2D ImageFilter array groups the players by unit, allowing the animation to apply distinct color filters that visually separate the "Offensive" and "Defensive" stars of the year.

## Image Filter Analysis

The keepColor filter modifies pixels by setting two of the three RGB channels to zero, isolating either the red or blue values to visually distinguish offensive and defensive units. The Grayscale filter calculates the average value of the red, green, and blue channels for each pixel and sets all three channels to that result, stripping away the color while keeping the image details.
